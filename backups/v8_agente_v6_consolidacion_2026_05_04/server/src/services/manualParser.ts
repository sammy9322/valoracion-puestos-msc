...
function cleanFunctions(text: string): string[] {
  ...
  // 1. Limpieza inicial: Normalizar espacios y eliminar ruidos de página
  ...
  // Insertamos un marcador especial [BREAK] antes de cada verbo de acción
  // SIEMPRE que esté precedido por un punto o sea el inicio del texto.
  let markedText = flow;
  for (const verbo of VERBOS_ACCION) {
    // Busca: (Punto + Espacio) + Verbo
    const regex = new RegExp(`(\\.\\s+)(${verbo}\\b)`, 'g');
    markedText = markedText.replace(regex, `$1[BREAK]$2`);
  }

  // 3. También separar por viñetas físicas si sobrevivieron
  markedText = markedText.replace(/[•\-\*]/g, '[BREAK]');

  // 4. Dividir y limpiar cada bloque
...
function finalizeSentence(s: string): string {
  ...
  // 1. Limpieza agresiva de puntos dobles y artefactos de extracción (. . .)
  let cleaned = s.replace(/\s+/g, ' ')
                 .replace(/\s*\.\s*\./g, '.')
                 .replace(/\.{2,}/g, '.')
                 .replace(/\.\s+\./g, '.')
                 .replace(/^[^a-zA-ZÁÉÍÓÚ]+/, '') // Quitar basura inicial
                 .trim();
  
  if (cleaned.length === 0) return "";
  
  // 2. Asegurar Mayúscula inicial
  cleaned = cleaned.charAt(0).toUpperCase() + cleaned.slice(1);
  
  // 3. Asegurar punto final único y limpio
