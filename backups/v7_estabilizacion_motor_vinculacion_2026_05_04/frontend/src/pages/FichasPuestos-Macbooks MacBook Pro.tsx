...
const FichasPuestos = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.area.trim()) {
      alert('El área organizacional es requerida');
      return;
    }
    if (!formData.descripcion_funciones.trim()) {
      alert('La descripción de funciones es requerida');
      return;
    }
    
    try {
      const res = await api.post('/puestos', formData);
      console.log('Puesto guardado:', res.data);
      setShowForm(false);
      setFormData({
  ...
  const handleDelete = async (id: string, nombre: string) => {
    if (!window.confirm(`¿Está seguro de eliminar la ficha: "${nombre}"? Esta acción se registrará en la auditoría y no se puede deshacer.`)) {
      return;
    }
    ...
    try {
      ...
      const msg = error.response?.data?.error || 'No se pudo eliminar la ficha';
      alert(`Error: ${msg}`);
    }
  };
        ...
        <div className="bg-card border rounded-lg shadow-sm overflow-hidden">
           <table className="w-full text-sm text-left">
             ...
             <tbody>
               {loading ? (
                 <tr>
                   <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">Cargando base de datos...</td>
                 </tr>
               ) : puestos.length === 0 ? (
                 <tr>
                   <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">No hay fichas registradas.</td>
                 </tr>
               ) : (
                 puestos.map(p => (
                   <tr key={p.id} className="border-b last:border-0 hover:bg-muted/10 cursor-pointer transition-colors">
                     ...
                     <td className="px-4 py-3 text-center">
                        <button 
                          ...
                          className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-full transition-colors flex items-center justify-center mx-auto"
                          title="Eliminar Ficha con registro en Auditoría"
                        >
                           <Trash2 size={16} />
                        </button>
                     </td>
