        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead>
            <tr className="bg-muted/10 border-b">
               <th className="px-4 py-3 font-medium text-muted-foreground w-40">Clasificación CAM</th>
           ...
           <tbody>
             {(data.items || []).map((i: any, index: number) => {
               const cat = getCategoriaByPuntos(i.puntos);
               return (
                <tr key={index} className="border-b last:border-0 hover:bg-muted/5">
                   <td className="px-4 py-3">
                      <div className="font-semibold text-foreground">{i.puesto}</div>
                   </td>
                   <td className="px-4 py-3 font-medium">
                      {cat ? (
                        <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded border bg-white shadow-sm">{cat.nombre}</span>
                      ) : (
            ...
            {(data.items || []).length === 0 && (
             ...
             <tr className="bg-muted/10 border-t-2 font-bold text-foreground">
                <td colSpan={2} className="px-4 py-3 font-bold text-right">Totales para Ecuación</td>
