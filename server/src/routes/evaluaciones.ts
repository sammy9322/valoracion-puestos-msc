...
router.get('/:id/report', async (req, res) => {
    try {
        ...
        // Recuperar intensidades desde la auditoria (datos.analisis)
        try {
          const auditRow: any = await prisma.auditoria.findFirst({
            where: { registro_id: id, tabla: 'Evaluacion', accion: 'EVALUACION_IA' },
            orderBy: { timestamp: 'desc' }
          });
          if (auditRow?.datos_despues) {
            const parsed = typeof auditRow.datos_despues === 'string' ? JSON.parse(auditRow.datos_despues) : auditRow.datos_despues;
            const analisis = parsed?.analisis || parsed?.reporte?.evaluacion;
            if (analisis) {
              const INT_FACTORS = ['dificultad', 'supervision', 'responsabilidad', 'condiciones', 'error', 'requisitos'];
              for (const f of INT_FACTORS) {
                const val = analisis[`${f}_intensidad`];
                if (val) (evaluacion as any)[`intensidad_${f}`] = val;
              }
            }
          }
        } catch (_e) {
          console.warn('[Evaluacion] Could not recover intensity from auditoria:', (_e as Error).message);
        }
